...
def sample_model_ISD_stations(da_model: xa.Dataset, gdf_ISD_state_stations: gpd.GeoDataFrame) -> pd.DataFrame:
    ...
    def process_station(row):
        df_station = (
            da_model.sel(longitude=row["LONGITUDE"], latitude=row["LATITUDE"], method="nearest")
    ...
    df_list = [process_station(row) for index, row in gdf_ISD_state_stations.iterrows()]

    df_model_daily_data = pd.concat(df_list)
    df_model_daily_data["forecast_day"] = (
                df_model_daily_data["forecast_date"] - df_model_daily_data["run_datetime"]).dt.days
